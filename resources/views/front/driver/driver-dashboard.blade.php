    ...
    <div class="container driver-dashboard">
        <div class="row">
            ...
            <div class="col-right">
                ...
                @endif
                <h1 class="title">Welcome {{ $driver->name }}</h1>
				...
				<div class="box dash-sec">
                    <div class="box-content">
						<div class="row">
						  <div class="col-6 col-md-4 box-add frst">
						   <div class="add-sec">
						    <a href="{{ url('driver-car-info') }}">
						  ...
						  <div class="col-6 col-md-4 box-add thrd">
						  <div class="add-sec">
						  <a href="{{ url('driver-availability-booking') }}">
						  <span class="img-sec"><img src="{{ asset('front/images/avibility.png') }}" alt="" /></span>
						  <span class="addlink">Book Availability</span>
						  </a>
						  </div>
						  </div>
						  <div class="col-6 col-md-4 box-add frth">
						  <div class="add-sec">
						  <a href="{{ url('driver-booking-history') }}">
			  ...
			  <a class="card-link collapsed" data-toggle="collapse" href="#collapseNine">My Info</a>
                    <div id="collapseNine" class="box-content collapse" data-parent="#accordion">
                        <ul>
                            <li>Name : <strong>{{ $driver->name }}</strong></li>
                            <li>Date Of Birth : <strong>{{Carbon\Carbon::createFromFormat('Y-m-d', $driver->dob)->format('d/m/Y')   }}</strong></li>
                            <li>sex : <strong>{{ $driver->gender }}</strong></li>
                            ...
                            <li>License number : <strong>{{ $driver->license_number }}</strong></li>
                        </ul>
                        <a class="edit" href="{{ route('EditDriverProfile') }}">edit</a>
                    </div>
                </div>
				
            </div>
        </div>
        <input type="hidden" id="driverNumber" value="{{$auth->mobile }}">
...
<script type="text/javascript">
    ...
    function verifyMobileNumber(bookingId) {
            var mobileNumber = $("#driverNumber").val();
            var number = "+91"+$("#driverNumber").val();
